...
from ..db import SessionLocal, Plan, Subscription, Payment, User, Setting, Log, PromoCode
...
async def plans():
    async with SessionLocal() as s:
        ...
        return [{"id": p.id, "title_ru": p.title_ru, "title_uk": p.title_uk, "days": p.days, "price_rub": p.price_rub} for p in result.scalars().all()]
...
async def profile(req: InitData):
    ...
    async with SessionLocal() as s:
        ...
        sub = (await s.execute(select(Subscription).where(Subscription.tg_id == tg_id, Subscription.active == True).order_by(Subscription.expires_at.desc()))).scalars().first()
...
async def payments_create(req: CreatePaymentReq):
    ...
    tg_id = int(user_data["id"])
    async with SessionLocal() as s:
        payment, url = await create_payment(s, tg_id=tg_id, plan_id=req.plan_id, provider=req.provider, promo=req.promo)
    ...
    return {"ok": True, "payment_id": payment.id, "checkout_url": url, "status": payment.status}

@router.post("/payments/mock-paid")
async def mock_paid(req: MockPaidReq):
    # Dev helper for manual tests.
    bot = Bot(settings.BOT_TOKEN)
    try:
...
async def cryptobot_webhook(request: Request):
    data = await request.json()
    payload = data.get("payload") or data.get("update", {}).get("payload") or data.get("result", {}).get("payload")
    if not payload:
        return {"ok": True}
    bot = Bot(settings.BOT_TOKEN)
    try:
...
async def promo_check(code: str):
    ...
    async with SessionLocal() as s:
        ...
        return {"ok": True, "code": promo.code, "discount_percent": promo.discount_percent, "uses_left": promo.uses_left}
